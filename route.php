...
$stmt = $conn->prepare('SELECT guide_id, safari_id, erp_n, date, route  FROM route WHERE id = ?');
$stmt-> bind_param("i", $unfinished_id);
$stmt-> execute();
$stmt-> store_result();
$stmt-> bind_result($route_rev['guide_id'], $route_rev['safari_id'], $route_rev['erp_n'], $route_rev['date'], $route_rev['route']);
$stmt-> fetch();
...
if (!is_null($_POST['safari']) && !empty($_POST['erp_n']) && is_numeric($_POST['erp_n']) && (strlen($_POST['erp_n']) == 7)&& !empty($_POST['time']) && !empty($_POST['route'])){
	...
	$date = $_POST['time'].':00';
	$stmt = $conn->prepare('INSERT INTO route (guide_id, safari_id, erp_n, date, route) VALUES (?, ?, ?, ?, ?);');
	$stmt-> bind_param('iisss', $_SESSION['guide_id'], $_POST['safari'], $erp_n, $date, $route);
	$stmt-> execute();
	$stmt-> close();
        $conn-> close();
	header('location:'.'kiitos.php');
